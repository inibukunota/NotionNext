...
const ExternalPlugin = props => {
  ...
  const ANALYTICS_ACKEE_TRACKER = siteConfig('ANALYTICS_ACKEE_TRACKER', null, NOTION_CONFIG)
  ...
  const COMMENT_TWIKOO_COUNT_ENABLE = siteConfig('COMMENT_TWIKOO_COUNT_ENABLE', null, NOTION_CONFIG)
  const RIBBON = siteConfig('RIBBON', null, NOTION_CONFIG)
  const CUSTOM_RIGHT_CLICK_CONTEXT_MENU = siteConfig('CUSTOM_RIGHT_CLICK_CONTEXT_MENU', null, NOTION_CONFIG)
  ...
  const AD_WWADS_BLOCK_DETECT = siteConfig('AD_WWADS_BLOCK_DETECT', null, NOTION_CONFIG)
  const CHATBASE_ID = siteConfig('CHATBASE_ID', null, NOTION_CONFIG)
  const COMMENT_DAO_VOICE_ID = siteConfig('COMMENT_DAO_VOICE_ID', null, NOTION_CONFIG)
  const AD_WWADS_ID = siteConfig('AD_WWADS_ID', null, NOTION_CONFIG)
  const COMMENT_ARTALK_SERVER = siteConfig('COMMENT_ARTALK_SERVER', null, NOTION_CONFIG)
  ...
  const COMMENT_GITTER_ROOM = siteConfig('COMMENT_GITTER_ROOM', null, NOTION_CONFIG)
  ...
  const CUSTOM_EXTERNAL_CSS = siteConfig('CUSTOM_EXTERNAL_CSS', null, NOTION_CONFIG)
  const CUSTOM_EXTERNAL_JS = siteConfig('CUSTOM_EXTERNAL_JS', null, NOTION_CONFIG)
  const ENABLE_NPROGRSS = siteConfig('ENABLE_NPROGRSS', false)
  const COZE_BOT_ID = siteConfig('COZE_BOT_ID')
  const HILLTOP_ADS_META_ID = siteConfig('HILLTOP_ADS_META_ID', null, NOTION_CONFIG)
  ...
  // FIX: The original code called loadExternalResource() directly in the render
  // body (inside an `if (isBrowser)` block). This ran on every render, not just
  // once on mount. Moved into a useEffect so resources are loaded exactly once.
  useEffect(() => {
    if (!isBrowser) return
    loadExternalResource('/css/custom.css', 'css')
    loadExternalResource('/js/custom.js', 'js')
    if (IMG_SHADOW) loadExternalResource('/css/img-shadow.css', 'css')
    if (ANIMATE_CSS_URL) loadExternalResource(ANIMATE_CSS_URL, 'css')
    if (CUSTOM_EXTERNAL_JS?.length > 0) {
      for (const url of CUSTOM_EXTERNAL_JS) loadExternalResource(url, 'js')
    }
    if (CUSTOM_EXTERNAL_CSS?.length > 0) {
      for (const url of CUSTOM_EXTERNAL_CSS) loadExternalResource(url, 'css')
    }
  }, [])

  // FIX: Analytics and widgets now defer until the user scrolls or the browser
  // is idle — whichever comes first. This frees up the main thread during the
  // critical render window and measurably improves Time to Interactive.
  const [pluginsReady, setPluginsReady] = useState(false)

  useEffect(() => {
    const activate = () => setPluginsReady(true)

    // Trigger on first scroll or pointer interaction
    window.addEventListener('scroll', activate, { once: true, passive: true })
    window.addEventListener('pointerdown', activate, { once: true })

    // Also trigger after idle or after a 4s fallback — whichever comes first
    let idleId
    if ('requestIdleCallback' in window) {
      idleId = requestIdleCallback(activate, { timeout: 4000 })
    } else {
      idleId = setTimeout(activate, 3000)
    }
  ...
  // FIX: The original useEffect had no dependency array, so eval(GLOBAL_JS) ran
  // after every single render. Added [] so it runs once on mount only.
  // The eval itself is retained for backwards compatibility with user configs
  // but the unnecessary re-execution on each render is eliminated.
  useEffect(() => {
  ...
  // Render only critical styles immediately. All heavy plugins wait until
  // pluginsReady is true (scroll / idle / 4s fallback).
  if (!pluginsReady) {
    return (
      <>
        <GlobalStyle />
        ...
        <>
          ...
          <script
            ...
            dangerouslySetInnerHTML={{
              __html: `window.chatbaseConfig = { chatbotId: "${CHATBASE_ID}" };`
            }}
          />
        </>
      )}
      ...
      {COMMENT_DAO_VOICE_ID && (
        <>
            ...
            dangerouslySetInnerHTML={{
              __html: `
                (function(i,s,o,g,r,a,m){
                  ...
                  if(m&&m.parentNode){m.parentNode.insertBefore(a,m)}else{s.head.appendChild(a)}
          ...
          <script
            async
            dangerouslySetInnerHTML={{
              __html: `daovoice('init',{app_id:"${COMMENT_DAO_VOICE_ID}"});daovoice('update');`
            }}
          />
        </>
      )}
      ...
      {COMMENT_GITTER_ROOM && (
        <>
          ...
          <script
            async
            dangerouslySetInnerHTML={{
              __html: `((window.gitter={}).chat={}).options={room:'${COMMENT_GITTER_ROOM}'};`
            }}
          />
        </>
      )}
          ...
          <script
            ...
            dangerouslySetInnerHTML={{
              __html: `
                ...
                gtag('js',new Date());
                gtag('config','${ANALYTICS_GOOGLE_ID}',{page_path:window.location.pathname});
              `
            }}
          />
        </>
...
const MouseFollow = dynamic(() => import('@/components/MouseFollow'), { ssr: false })
